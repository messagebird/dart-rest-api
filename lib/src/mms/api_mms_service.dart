...
class ApiMmsService extends BaseService implements MmsService {
  ...
  @override
  Future<MmsMessage> create(MmsMessage message) => post('/mms',
          body: message.toMap(send: true))
      .then((response) => Future.value(
          response?.body == null ? null : MmsMessage.fromJson(response.body)));

  @override
  Future<List<MmsMessage>> list({int limit, int offset}) =>
      get('/mms', body: {'limit': limit, 'offset': offset}).then((response) =>
          Future.value(response?.body == null
              ? null
              : MmsMessage.fromJsonList(response.body)));
