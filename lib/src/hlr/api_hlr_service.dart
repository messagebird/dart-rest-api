...
class ApiHlrService extends BaseService implements HlrService {
  ...
  Future<Hlr> create(int msisdn, {String reference}) =>
      post('/hlr', body: {'msisdn': msisdn, 'reference': reference}).then(
          (response) => Future.value(
              response?.body == null ? null : Hlr.fromJson(response.body)));

  @override
  Future<Hlr> read(String id) =>
      get('/hlr/$id').then((response) => Future.value(
          response?.body == null ? null : Hlr.fromJson(response.body)));
  ...
  @override
  Future<List<Hlr>> list() => get('/hlr').then((response) => Future.value(
      response?.body == null ? null : Hlr.fromJsonList(response.body)));
}
